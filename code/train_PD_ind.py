...
def import_data():
    ...
    return PS_seq,DS_doid,adjPD,PD_ben_ind_label,rel_GB
...
if __name__ == "__main__":
    ...
    #load data
    PS_seq, DS_doid,adjPD,PD_ben_ind_label,rel_GB = import_data()
    ...
    # load index
    train_index = np.where((PD_ben_ind_label>0) | (-11>PD_ben_ind_label))
    test_index = np.where((PD_ben_ind_label<0) & (-11<PD_ben_ind_label))
    train_pos_index = np.where((PD_ben_ind_label ==1))
    ...
    #keep train_label constant, improve test_label with rel_GB 
