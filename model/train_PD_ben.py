...
import sys
sys.path.append('/home/houjialu/anaconda3/lib/python3.7/site-packages/torch')
...
def import_data():
    ...
    PD_fold_label = np.load(file_name + '/PD_fold5.npy')
    adjPD=np.load(file_name + '/adjPD.npy')
    return PS_seq,DS_doid,PD_fold_label,adjPD
...
if __name__ == "__main__":
    fold=5
    PS_seq, DS_doid, PD_fold_label,adjPD = import_data()
    ...
    for i in range(fold):
        ...
        train_index = np.where(
            (PD_fold_label != (i + 1)) & (PD_fold_label != -(i + 1)) & (PD_fold_label != 0))
        test_index = np.where((PD_fold_label == (i + 1)) | (PD_fold_label == -(i + 1)))
        ...
        train_data = {
            ...
            "test_annotation": test_annotation[test_index].to(device),
        ...
        s = df_Y_pred.where(test_annotation.astype(bool)).stack()
