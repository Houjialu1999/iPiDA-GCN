...
class GCMC(nn.Module):
    ...
    """
    def __init__(self, n_prot, n_term,X_prot, X_term):
        """
        :param n_prot: the number of piRNA
        ...
        self.bm_input_x = norm.BatchNorm(X_prot.shape[1])
        self.bm_input_y = norm.BatchNorm(X_term.shape[1])

        self.dense_x = nn.Linear(X_prot.shape[1], 800)
        self.dense_y = nn.Linear(X_term.shape[1], 800)
        ...
        # self.gcn_xy3 = dense.DenseGraphConv(800, 800)
        # self.gcn_x3 = dense.DenseGCNConv(800, 800)
        # self.gcn_y3 = dense.DenseGCNConv(800, 800)
        # self.bm_xy3 = norm.BatchNorm(800)
        # self.bm_x3 = norm.BatchNorm(800)
        # self.bm_y3 = norm.BatchNorm(800)

        # self.gcn_xy4 = dense.DenseGraphConv(800, 800)
        # self.gcn_x4 = dense.DenseGCNConv(800, 800)
        # self.gcn_y4 = dense.DenseGCNConv(800, 800)
        # self.bm_xy4 = norm.BatchNorm(800)
        # self.bm_x4 = norm.BatchNorm(800)
        # self.bm_y4 = norm.BatchNorm(800)
        ...
        # self.linear_x4 = nn.Linear(100, 50)
        # self.linear_y4 = nn.Linear(100, 50)
        ...
        self.bm_lx3 = norm.BatchNorm(100)
        self.bm_ly3 = norm.BatchNorm(100)
    ...
    def forward(self, X_prot, X_term, A_prot, A_term, A_rel):
        ...
        # XY = t.squeeze(F.leaky_relu(self.gcn_xy3(t.cat((X, Y)), A_rel)))
        # XY = self.bm_xy3(XY)
        # X, Y = t.split(XY, (m, n))
        # X = t.squeeze(F.leaky_relu(self.gcn_x3(X, A_prot)))
        # Y = t.squeeze(F.leaky_relu(self.gcn_y3(Y, A_term)))
        # X = self.bm_x3(X)
        # Y = self.bm_y3(Y)

        # XY = t.squeeze(F.leaky_relu(self.gcn_xy4(t.cat((X, Y)), A_rel)))
        # XY = self.bm_xy4(XY)
        # X, Y = t.split(XY, (m, n))
        # X = t.squeeze(F.leaky_relu(self.gcn_x4(X, A_prot)))
        # Y = t.squeeze(F.leaky_relu(self.gcn_y4(Y, A_term)))
        # X = self.bm_x4(X)
        # Y = self.bm_y4(Y)
        ...
        # X = self.bm_lx3(X)
        # Y = self.bm_ly3(Y)
        # X = F.relu(self.linear_x4(X))
        # Y = F.relu(self.linear_y4(Y))
